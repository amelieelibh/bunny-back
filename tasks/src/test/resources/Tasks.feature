...
Feature: Tasks Tests for Bunny Studio
  ...
  Scenario Outline: Retrieving existing task
    ...
    When I call the service with GET method and 1 as id
    Then I validate that userId is 1 and desc "My first task" and status "TODO"

  Scenario Outline: Retrieving list of tasks from user
    Given Many existent tasks for user 1
    When I call the service with GET method and 1 as userId
  ...
  Scenario Outline: Deleting existing task
    Given An existent task with id 5 and desc "This is an existent task"
    When I call the service with DELETE method and id 5
    And I call the service with GET method and 5 as id
    Then I validate that task 5 doesnt exist
